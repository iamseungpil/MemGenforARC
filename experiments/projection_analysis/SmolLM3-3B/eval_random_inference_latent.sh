#!/bin/bash
#
# Evaluation script: Random Inference Latent Only - SmolLM3-3B
#
# This script evaluates the model with:
# - load_prompt_query_latents: true (prompt latents LOADED)
# - load_inference_query_latents: false (inference latents RANDOM)
#
# Purpose: Ablation study - measure contribution of inference latents

export CUDA_VISIBLE_DEVICES=1
export WANDB_MODE=online
export WANDB_ENTITY=gistdslab
export WANDB_PROJECT=memgen_reproduce
export WANDB_RUN_NAME="eval_gsm8k_smollm3-3b_random-inference-latent"
...
LOAD_PROMPT_QUERY_LATENTS=True    # Load trained prompt latents
LOAD_INFERENCE_QUERY_LATENTS=False # Keep inference latents RANDOM

# Output log
LOG_FILE="./logs/eval_random_inference_latent_$(date +%Y%m%d_%H%M%S).log"
mkdir -p ./logs

echo "========================================"
echo "Evaluation: Random Inference Latent (SmolLM3-3B)"
echo "========================================"
