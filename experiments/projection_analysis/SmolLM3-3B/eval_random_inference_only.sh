#!/bin/bash
#
# Evaluation script: Random Inference Latent Only (No Prompt Aug) - SmolLM3-3B
#
...
# - load_inference_query_latents: false (inference latents RANDOM)
#
# Purpose: Test effect of random inference latents without any prompt augmentation

export CUDA_VISIBLE_DEVICES=1
export WANDB_MODE=online
export WANDB_ENTITY=gistdslab
export WANDB_PROJECT=memgen_reproduce
export WANDB_RUN_NAME="eval_gsm8k_smollm3-3b_random-inference-only"
...
# Latent loading settings
LOAD_INFERENCE_QUERY_LATENTS=False # Keep inference latents RANDOM

# Output log
LOG_FILE="./logs/eval_random_inference_only_$(date +%Y%m%d_%H%M%S).log"
mkdir -p ./logs

echo "========================================"
echo "Evaluation: Random Inference Only (SmolLM3-3B)"
echo "========================================"
echo "Model: ${MODEL_NAME}"
echo "Weaver checkpoint: ${LOAD_WEAVER_PATH}"
echo "max_prompt_aug_num: ${MAX_PROMPT_AUG_NUM} (NO prompt aug)"
echo "max_inference_aug_num: ${MAX_INFERENCE_AUG_NUM}"
echo "skip_lora: ${SKIP_LORA}"
echo "load_inference_query_latents: ${LOAD_INFERENCE_QUERY_LATENTS} (RANDOM)"
