#!/bin/bash
#
# Evaluation script: Inference Augmentation Only (No Prompt Augmentation) - SmolLM3-3B
#
...
#
# Purpose: Test whether prompt augmentation contributes to performance

export CUDA_VISIBLE_DEVICES=0
export WANDB_MODE=online
export WANDB_ENTITY=gistdslab
export WANDB_PROJECT=memgen_reproduce
export WANDB_RUN_NAME="eval_gsm8k_smollm3-3b_inference-only"
...
# Output log
LOG_FILE="./logs/eval_inference_only_$(date +%Y%m%d_%H%M%S).log"
mkdir -p ./logs

echo "========================================"
echo "Evaluation: Inference Only (SmolLM3-3B)"
echo "========================================"
echo "Model: ${MODEL_NAME}"
echo "Weaver checkpoint: ${LOAD_WEAVER_PATH}"
echo "max_prompt_aug_num: ${MAX_PROMPT_AUG_NUM} (DISABLED)"
echo "max_inference_aug_num: ${MAX_INFERENCE_AUG_NUM} (ENABLED)"
